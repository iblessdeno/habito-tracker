...
import { Calendar } from '@/components/ui/simple-calendar'; // Using simple calendar
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    // Only run after component is mounted and we have selected a habit
    if (!mounted || !selectedHabit) return;
    ...
    // Cleanup on unmount or when selected habit changes
    return () => {
      clearTimeout(timer);
      document.querySelectorAll(".completion-indicator").forEach(el => el.remove());
    };
  }, [mounted, selectedHabit, completionData, getHabitColor]);
